...
SRCDEP = $(DEPDIR)/quadpack.f90 $(DEPDIR)/splines.f90
OBJDEP = $(SRCDEP:%.f90=%.o)
...
#######OPTIONS settings###########
OPT := -I$(DEPDIR) -I$(SRCDIR) -I$(DECOMPDIR) $(FFLAGS) 
LINKOPT := $(FFLAGS)
...
xcompact3d : $(OBJDECOMP) $(OBJDEP) $(OBJ) 
	$(FC) -o $@ $(LINKOPT) $(OBJDECOMP) $(OBJDEP) $(OBJ) $(LIBFFT) $(LIBIO)
...
clean:
	...
	rm -f $(DEPDIR)/*.o $(DEPDIR)/*.mod $(DEPDIR)/*.smod
